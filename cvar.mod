...
# Objective function
minimize cvar: (1 / (n)) * sum {j in 1..n} ((zeta + (1/(1 - beta)) * y[j])) + sum {inst in 1..nForward} (kappaForward[inst] * costForward[inst]*(alphaLong[inst]+alphaShort[inst])) + sum {inst in 1..nPut} (kappaPut[inst]*costOption[inst]*alphaPut[inst]);
...
# Constraint on portfolio effect
subject to portfolioEffect {i in 1..n}:
