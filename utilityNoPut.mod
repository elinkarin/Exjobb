...
# Objective function
minimize cvar: lambda * (zeta + (1 / (n * (1 - beta))) * sum {j in 1..n} (y[j])) - (1-lambda)/n * sum {i in 1..n} (log(v[i])) + sum {inst in 1..nForward} (kappaForward[inst]*costForward[inst]*(alphaLong[inst]+alphaShort[inst]));
